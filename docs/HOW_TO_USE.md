...
4. **批量上传图源**：
   - 支持直接拖拽几十甚至上百张图片批量上传；
   - 自动读取图片原始宽高并生成低带宽缩略图；
   - 支持拖拽卡片调整页面顺序（出现插入位置指示线后松手即可），或用「**按文件名重排**」按素材文件名一键自然排序（数字按数值比较，`1、2、10` 不会错排；覆盖手动排序前会弹窗确认）。
...
### 8. 页级工作状态筛选与「继续」直达（v0.2.8）
- 筛选栏新增「**工作状态**」下拉：**未标号**（还没有任何图钉标号）/ **待翻译**（有标号但译文没填满，含部分翻译的页）/ **待嵌字** / **已嵌字**，与关键字搜索、「只看我认领的」组合生效；
- 详情页「**继续翻译 / 继续嵌字**」按钮：一键跳进全空间第一个待处理页的编辑器，全部完成时按钮自动置灰并提示；
- 标注编辑器内也有「**下一未译 / 下一嵌字**」按钮：按页序从当前页向后找，到末页自动**绕回开头**不漏页；有未保存的修改会先自动保存再跳转。

### 9. 页序增强：按文件名重排 + 拖拽排序（v0.2.8）
- 「**按文件名重排**」：按素材文件名自然排序（数字按数值比较），无文件名的图片回退条目标题，都没有则保持在末尾原顺序；执行前弹窗确认——**会覆盖手动排序**；
- **卡片拖拽排序**：直接把卡片拖到目标位置，出现插入指示线后松手即可；超过 30 张启用虚拟滚动时，请先滚动到目标附近再拖（仅支持拖到当前可见窗口内的卡片位置）；
- 两种排序与卡片 ↑↓ 微调一样，都只在「**全部视图**」（清空搜索 / 认领 / 工作状态筛选）下可用，避免对筛选子集排序打乱全空间页序。
