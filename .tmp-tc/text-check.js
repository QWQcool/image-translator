...
function checkEllipsis(text, issues) {
    const re = /[。.…]{3,}/g;
    let match;
...
function checkBlankLines(text, issues) {
    // 三个及以上连续换行 = 至少两个空行
    if (/(?:\n[ \t\u3000]*){2,}\n/.test(text)) {
        issues.push({
            rule: '多余空行',
...
/** 规则 3：行首/行尾空白、行首全角空格 */
function checkEdgeSpaces(text, issues) {
 ...
 */
function findMixedPeriodIndex(text) {
    let from = 0;
    for (;;) {
...
function checkMixedPunctuation(text, issues) {
...
/** 规则 5：引号不配对（“”、『』、《》数量不等），仅提示 */
function checkQuotes(text, issues) {
    const pairs = [
        ['“', '”', '引号'],
        ['『', '』', '直角引号'],
        ['《', '》', '书名号'],
...
function checkRepeatedPunctuation(text, issues) {
 ...
 */
function findConvertiblePeriod(text) {
    const chars = [...text];
    for (let i = 0; i < chars.length; i += 1) {
        ...
        if (chars[i - 1] === '.' || chars[i + 1] === '.')
            continue;
        if (CJK_CHAR.test(chars[i - 1] ?? '') || CJK_CHAR.test(chars[i + 1] ?? ''))
            return i;
    }
    return -1;
}
...
function checkHalfWidthPunctuation(text, issues) {
    const direct = /[,:;!?]/.exec(text);
    const periodIndex = findConvertiblePeriod(text);
    if (!direct && periodIndex < 0)
        return;
    issues.push({
        rule: '半角标点',
