...
| **嵌字与修图** | 涂改层 + 文字层合成，导出已嵌字成品；支持 LabelPlus 分组样式预设；**AI 智能去字修图**（集成 LaMa / IOPaint 模型一键消除原图文字，支持笔迹蒙版 / 选区蒙版 / 液化推挤 / 非破坏背景色阶） |
| **AI 批量与流水线** | 支持多选图片直接调起 **AI 批量处理** 或空间一键机翻；OCR + 视觉检测 + 翻译大模型流水线，支持术语表（Glossary）Prompt 上下文注入 |
| **协作与效率** | **存疑跨页汇总**（详情页「存疑清单」带数量徽标，每行直达对应标注）；**空间进度统计 chips**（页数 / 标号 / 已填译文 / 存疑一目了然）；**全空间终检**（空译文 / 存疑未清 / 标点硬伤分组报告，逐条可跳转）；**协作通知扩展**（进度流转、存疑标记自动提醒空间参与者，排除操作者、聚合防刷屏）；**术语命中提示**（原文命中空间术语表即显示译词 chips 一键插入）；**翻译记忆（TM）**（同原文历史译文「记忆命中 → 采用」一键回填）；**页级认领**（认领 / 接手 / 取消 + 「只看我认领的」筛选与操作日志留痕）；**阅读器成品切换**（原图 \| 成品 对照阅读，成品页不叠加译文） |
| **工作状态与续作** | **页级工作状态筛选**（未标号 / 待翻译 / 待嵌字 / 已嵌字，与搜索、认领筛选组合生效）；**「继续翻译 / 继续嵌字」直达**（详情页一键跳到第一个未完成页，全部完成自动置灰）；**编辑器内逐页推进**（「下一未译 / 下一嵌字」按页序向后找、到尾绕回开头，未保存修改先自动保存再跳转） |
| **页序管理** | **按文件名一键重排**（数字按数值比较，`1、2、10` 不会错排；无文件名回退条目标题；覆盖手动排序前弹窗确认）；**网格拖拽排序**（卡片直接拖到目标位置、插入指示线实时显示；虚拟滚动下支持拖到当前可见窗口内）；排序操作在搜索 / 认领 / 工作状态筛选生效时统一禁用，保证全空间顺序语义 |
| **阅读模式** | 空间详情页「阅读」进入全屏阅读器；单页/对页、正序/倒序、适应屏幕、显示标注译文叠加；该页已有嵌字成品时可在「原图 \| 成品」间切换对照 |
...
- **术语命中提示**：编辑器标号面板中，原文命中术语表时在原文框下方显示「译词 → 备注」chips，
  点击即把译词追加进译文，专有名词不再手打
- **翻译记忆（TM）**：编辑器加载时自动拉取全空间「原文 → 译文」对照（同原文取最新一版）；
  某个标号原文非空且译文为空时提示「记忆命中：<译文> [采用]」，点击整段写入（可撤销），
  多页重复台词、拟声词不用翻第二遍
...
- **全空间终检**：详情页「终检」按钮把**空译文 / 存疑未清 / 标点硬伤**三类问题分组汇总
  （仅报可自动修的硬伤类：省略号 / 多余空行 / 首尾空白，与单页检查同口径），
  每行可点击直达对应标注；超过 500 页自动截断提示，交稿前跑一遍安心交稿
...
- **画布变换手柄**：选中文字层显示 **8 向手柄**（角/上下 = 拖拽等比缩放、左右 = 拖拽限宽）
  + **旋转手柄**（绕包围盒中心）；**双击文字层画布就地编辑**（Enter 提交 / Esc 取消）；
  拖拽实时预览、松手入撤销栈并同步协作端；只读模式自动禁用
- **变换**：任意角度旋转（绕包围盒中心）、非破坏缩放（0.2~4×）、渐变填充
  （双色线性，横排纵向/竖排横向，与描边阴影共存）
- **特效**：描边（颜色+比例宽度）与阴影（颜色/模糊/偏移），逐层可选零迁移
- **去字**：画笔/橡皮 + 仿制图章（Alt 取源复制背景纹理）+ **液化**（向前变形平滑推挤背景，
  结果画进涂改层非破坏，大小=影响域、不透明度=强度）+ 三级蒙版来源——
  **笔迹蒙版**（涂改层笔迹即去字区域）、**选区去字**（矩形/套索选区光栅化，异形气泡友好）、
  标号框兜底；引擎链 LaMa → AI（含漂移校验）→ telea/洋葱剥皮确定性兜底
- **笔刷引擎**：**硬度**（0~100 连续渐变，替代旧「软边」开关）/**流量**（<100 低流量逐章堆积
  喷枪感，笔画总浓度封顶在不透明度）/**间距**（1~500%）/**平滑**（输入点 EMA 稳定器）；
  **压感**（大小/不透明度独立开关，数位板 p∈[0,1]→[0.65,1]×，鼠标恒按 0.5）；
  **散布**（0~500%，偏移幅度上限 = 笔刷大小 × 散布%，逐轴独立）与**大小抖动**（0~100%）；
  **椭圆笔尖**（角度 0~180° + 圆度 1~100，软边渐变同步椭圆化）；
  全部参数随协作矢量笔画广播、多人实时重放逐像素一致（散布/抖动用落笔种子 PRNG，重放零随机源），
  旧笔画数据向后兼容；图章保持硬边正圆
- **背景调整**：非破坏色阶——亮度/对比度/黑场/白场/Gamma 五滑条，SVG LUT 精确预览
  （与导出逐像素一致），仅作用于背景层（笔迹与文字颜色不被调），全默认零像素差异
- **图层**：Shift 多选、批量改样式、五模式对齐、显隐/上移/删除，撤销重做全程可回退
  （栈深按图片大小动态收缩：小图 100 步 / 大图 30 步）
- **导出**：PNG（所见即所得）、**PSD**（文字层升级为 **ag-psd 原生 TypeTool**——PS 打开直接编辑
  文字无需栅格化；渐变/阴影/纵中横排层自动栅格兜底，混合输出；批量分层 PSD 压缩包同款引擎）、
  保存成品入库（见「成品管理」）
...
- **通知中心**：顶栏铃铛（60s 轮询），单条点击直达对应标注页；删条目时相关评论与通知一并清除
- **协作事件通知**（自动，只发给空间其他参与者，操作者本人与单人场景不发）：
  - **进度流转**：有人把空间进度推进到新状态时通知（如「alice 把《某话》进度推进到 已校对」）；
  - **存疑标记**：保存标注时新出现存疑标号，按页汇总一条通知（如「bob 在《某话》的「p1」新标记 2 个存疑标号」），
    一页一条防刷屏；
  - 参与者 = 在该空间保存过标注、发过评论的人 ∪ 空间创建者（去重，各来源上限 500）
